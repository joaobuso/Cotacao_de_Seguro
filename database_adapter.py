...
class DatabaseAdapter:
    ...
    def save_quotation(self, phone: str, client_data: Dict, pdf_path: str,
                    status: str = "completed", completed_by: str = "bot"):
        """
        Salva cotação no formato que o PORTAL utiliza
        (collection quotations do MongoDB)
        """
        try:
            quotation = {
                "phone": phone,
            ...
            # Aqui chamamos o método do DatabaseManager original
            if hasattr(self.db_manager, "quotations_collection"):
                self.db_manager.quotations_collection.insert_one(quotation)

            logger.info(f"Cotação registrada para o portal: {phone}")

            return True

        except Exception as e:
            logger.error(f"Erro ao salvar cotação para portal: {str(e)}")
            return False
